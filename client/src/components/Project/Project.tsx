...
const Project = (props: Props) => {
  ...
  const trans = (x: number, y: number, s: number) =>
    `perspective(3000px) rotateX(${x / 2}deg) rotateY(${y / 2}deg)`
  const [move, set] = useSpring(() => ({
    xys: [0, 0, 1],
  ...
  return (
    ...
    >
      <div onClick={props.complete && onOpen}>
        <animated.img className="thumnail" src={props.image} alt={props.name} />
      </div>
      {!props.complete && (
        <p className="not-complete">-- Work in Progress --</p>
      )}
          ...
          <ModalFooter className="more-links">
            <Flex w="100%" justify="space-between" align="flex-end">
              ...
              <Stack>
                {props.git && (
                  <Box>
                    <a href={props.git} target="_blank" rel="noopener">
                      <Button {...btnStyles}>
                        {' '}
                        See Github
                        <Image
